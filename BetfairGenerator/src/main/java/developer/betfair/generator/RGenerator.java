...
public class RGenerator implements genDataTypeListOf, GeneratorInterface {
    ...
    /**
     * Construct by generating the R version of
     *
     * @param i
     * @param outDir
     * @param api
    ...
    public RGenerator(Interface i, String outDir, String api)
            throws IOException {
        generate(i, outDir, api);
    }

    /**
     * Generate the R version of
     *
     * @param i
     * @param outDir
     * @param api
     * @throws IOException
     */
    public final void generate(Interface i, String outDir, String api)
            throws IOException {
        this.api = api;
        Map<String, String> simpleToPrimitiveTypeMap = new HashMap<>();

        new File(outDir).mkdirs();

        try (FileOutputStream fbetfair = new FileOutputStream(String.format(
                "%s/%s.R", outDir, i.name))) {
    ...
    /**
     * Generate the JSON to R wrapper implementation of the operation
     * 
     * @param buff
     * @param o
     */
    public void genOperationToRConversion(StringBuffer buff, Operation o) {
        StringBuilder funcbuff = new StringBuilder();
